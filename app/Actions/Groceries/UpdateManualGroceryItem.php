...
final class UpdateManualGroceryItem
{
    /** @param array{name: string, quantity_description?: string|null, category: string} $data */
    public function handle(User $user, GroceryItem $item, array $data): GroceryItem
    {
        Gate::forUser($user)->authorize('update', $item);
        if ($item->source !== GroceryItemSource::Manual) {
            throw new InvalidArgumentException('Generated grocery items cannot be permanently edited.');
        }
