    ...
    <div class="grid gap-4 lg:grid-cols-2">
        @forelse ($this->recommendations as $result)
            <flux:card wire:key="recommendation-{{ $result->recipe->id }}" class="space-y-4">
                <div class="flex items-start justify-between gap-4"><div><div class="flex items-center gap-2"><flux:heading size="lg">{{ $result->recipe->name }}</flux:heading>@if ($result->isFavourite)<flux:badge color="amber">Favourite</flux:badge>@endif</div><flux:text>{{ $result->servings }} servings</flux:text></div><div class="text-right"><div class="text-2xl font-semibold">{{ $this->scoreLabel($result) }}</div><div class="text-xs text-zinc-500">recommendation score</div></div></div>
