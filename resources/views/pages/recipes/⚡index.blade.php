...
new #[Title('Recipes')] class extends Component {
    ...
    #[Url] public bool $favouritesOnly = false;
    ...
    public function updatedFavouritesOnly(): void { $this->resetPage(); }

    public function addFavourite(int $recipeId, AddRecipeFavourite $addFavourite): void
    {
        $addFavourite->handle($this->user(), $this->ownedRecipe($recipeId));
        unset($this->recipes);
    }

    public function removeFavourite(int $recipeId, RemoveRecipeFavourite $removeFavourite): void
    {
        $removeFavourite->handle($this->user(), $this->ownedRecipe($recipeId));
        unset($this->recipes);
    }
    ...
    public function recipes(): LengthAwarePaginator
    {
        return Recipe::query()->whereBelongsTo($this->user())->active()->search($this->search)
            ->when($this->favouritesOnly, fn ($query) => $query->favouritedBy($this->user()))
            ...
            ->withExists(['favouritedByUsers as is_favourite' => fn ($query) => $query->whereKey($this->user()->id)])
    ...
    private function ownedRecipe(int $recipeId): Recipe { return Recipe::query()->whereBelongsTo($this->user())->findOrFail($recipeId); }
...
<section class="w-full space-y-6">
    ...
    <div class="grid gap-3 md:grid-cols-4">
        ...
        <flux:field variant="inline"><flux:label>Favourites only</flux:label><flux:switch wire:model.live="favouritesOnly" /></flux:field>
    </div>
    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        @forelse ($this->recipes as $recipe)
            <flux:card wire:key="recipe-{{ $recipe->id }}" class="overflow-hidden p-0!">
                ...
                <div class="space-y-4 p-5"><div class="flex items-start justify-between gap-3"><div><flux:heading size="lg">{{ $recipe->name }}</flux:heading><flux:text class="mt-1">{{ $recipe->default_servings }} servings · {{ $recipe->ingredients_count }} ingredients</flux:text></div>@if ($recipe->is_favourite)<flux:button wire:click="removeFavourite({{ $recipe->id }})" wire:loading.attr="disabled" size="sm" variant="ghost" aria-label="Remove {{ $recipe->name }} from favourites">Favourited</flux:button>@else<flux:button wire:click="addFavourite({{ $recipe->id }})" wire:loading.attr="disabled" size="sm" variant="ghost" aria-label="Add {{ $recipe->name }} to favourites">Favourite</flux:button>@endif</div><div class="flex flex-wrap gap-2">@foreach ($recipe->categories as $recipeCategory)<flux:badge>{{ $recipeCategory->name }}</flux:badge>@endforeach @foreach ($recipe->tags as $recipeTag)<flux:badge color="zinc">{{ $recipeTag->name }}</flux:badge>@endforeach</div><div class="flex flex-wrap justify-end gap-2"><flux:button wire:click="planDinner({{ $recipe->id }})" size="sm" variant="primary">Plan dinner</flux:button><flux:button :href="route('recipes.show', $recipe)" wire:navigate size="sm">View</flux:button><flux:button :href="route('recipes.edit', $recipe)" wire:navigate size="sm" variant="ghost">Edit</flux:button><flux:button wire:click="archive({{ $recipe->id }})" wire:confirm="Archive this recipe?" size="sm" variant="ghost">Archive</flux:button></div></div>
            </flux:card>
        @empty
